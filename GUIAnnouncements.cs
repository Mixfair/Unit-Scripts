...
namespace Oxide.Plugins
{
    ...
    [Description("Creates announcements with custom messages across the top of every player's screen in a banner.")]
    ...
    class GUIAnnouncements : RustPlugin
    {
        ...
        private bool PluginAirstrike = false;
        #endregion
        ...
        private void LoadGUIAnnouncementsConfig()
        {
            ...
            if (restartTimes.Contains("24:00:00"))
            {
                restartTimes[restartTimes.IndexOf("24:00:00")] = "00:00:00";
                ...
                Config["Public Restart Announcements", "Restart Times"] = restartTimes;
                ConfigUpdated = true;
            }
            ...
            welcomeAnnouncementDelay = GetConfig("Private Welcome Announcements", "Delay", 0f);
        ...
        void OnAirdrop(CargoPlane plane, Vector3 location)
        {
            ...
            //if (Airstrike.Call<bool>("isStrikePlane", plane))
            //    isAirstrike = true;
                ...
                if (args?.Length > 1)
                {
                    ...
                    if (targetedPlayer != null)
                    {
                        if (!Exclusions.ContainsKey(targetedPlayer.userID))
                            ...
                            SendReply(player, Lang("IsExcluded", player.UserIDString).Replace("{playername}", targetedPlayer.displayName));
                    }
                ...
                if (arg?.Args?.Length > 1)
                {
                    ...
                    if (targetedPlayer != null)
                    {
                        if (!Exclusions.ContainsKey(targetedPlayer.userID))
                            ...
                            SendReply(arg, Lang("IsExcluded", arg.Connection?.userid.ToString()).Replace("{playername}", targetedPlayer.displayName));
                    }
        ...
        void cmdAnnounceTest(BasePlayer player, string cmd)
        {
            if (player.net.connection.authLevel > 0 || hasPermission(player, PermAnnounce))
            {
                if (!Exclusions.ContainsKey(player.userID))
                    ...
                    SendReply(player, Lang("YouAreExcluded", player.UserIDString));
            }
        }
        ...
        {
            if (player.net.connection.authLevel > 0 || hasPermission(player, PermAnnounce) || hasPermission(player, PermAnnounceGetNextRestart))
            {
