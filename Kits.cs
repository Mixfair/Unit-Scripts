...
namespace Oxide.Plugins
{
    ...
    { 
        ...
        static string dataDirectory = $"file://{Interface.Oxide.DataDirectory}{Path.DirectorySeparatorChar}Kits{Path.DirectorySeparatorChar}";
        ...
        void Loaded()
        {
            ...
            lang.RegisterMessages(messages_en, this);
        ...
        {  
            ...
            if (!string.IsNullOrEmpty(BackgroundURL)) {
                if(!BackgroundURL.Contains("http"))
                    BackgroundURL = $"{dataDirectory}{BackgroundURL}.png";
                if (ImageLibrary)
                ...
                },
{
        ...
        Dictionary<string, string> messages_en = new Dictionary<string, string>()
        {
            {"title", "Kits: " },
            {"Name", "Name" },
        ...
        Dictionary<string, string> messages_ru = new Dictionary<string, string>()
        {
            {"title", "Киты: " },
            {"Name", "Имя" },
            ...
            {"AddKit", "Добавить кит" },
            {"Close", "Закрыть" },
            {"NoKitsFound", "Все доступные киты в меню!" },
            {"AddKitToMenu", "Выберите кит для добавления в меню" },
            {"KitCooldown", "Перезарядка: {0}" },
            {"Unavailable", "Недоступно" },
            {"Last", "Последняя" },
            {"Next", "Дальше" },
            {"Back", "Назад" },
            {"First", "Первая" },
            {"RemoveKit", "Удалить кит?" },
            {"KitUses", "Использования: {0}" },
            {"NoInventorySpace", "Не хватает места в инвентаре для получения!" },
            {"Unlimited", "Бесконечно" },
            {"None", "None" },
            {"KitRedeemed", "Кит получен" },
            {"Emptykitname", "Пустое имя кита" },
            {"KitExistError","Этот кит не существует"},
            {"CantRedeemNow","Вы не можете получить кит сейчас"},
            {"NoAuthToRedeem","Вам не хватает доступа для получения кита"},
            {"NoPermKit","Вам не хватает привелегий для получения кита"},
            {"NoRemainingUses","Вы уже получили все возможные киты"},
            {"CooldownMessage","Вы должны подождать {0} секунд, чтобы использовать кит"},
            {"NPCError","Вы должны найти НПС, который выдаст вам этот кит."},
            {"PastingError", "Something went wrong while pasting, is CopyPaste installed?"},
            {"NoKitFound", "Кита не существует" },
        };
        ...
        void BackgroundPanel(BasePlayer player) 
        {
            ...
            var element = UI.CreateOverlayContainer(PanelBackground, "0.43 0.41 0.36 0","0 0", "1 1", true);
            UI.CreatePanel(ref element, PanelBackground, ".1 .1 .1 0.7", "0 0", "1 1");
        ...
        void CreateKitEntry(BasePlayer player, ref CuiElementContainer element, string panel, float[] pos, string entry)
        {
            ...
            var bg = "http://i.imgur.com/xxQnE1R.png";
            if (ImageLibrary){
                image = TryForImage(kit.name.Replace(" ", ""));
                bg = TryForImage("Background");
            }
                
            UI.CreatePanel(ref element, PanelKits, "0 0 0 0.4", $"{pos[0]} {pos[1]}", $"{pos[2]} {pos[3]}");
            //UI.LoadImage(ref element, PanelKits, bg, $"{pos[0]} {pos[1]}", $"{pos[2]} {pos[3]}");
        ...
        {
            Vector2 position = new Vector2(0.015f, 0.25f);
            ...
            { 
                offsetX = 0.1f + (-0.01f - dimensions.x) * number;
            }
