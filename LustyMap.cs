...
{ 
    ...
    class LustyMap : RustPlugin
    {
        ...
        class MapUser : MonoBehaviour
        {
            ...
            public Dictionary<string, MapMarker> localMarkers;
            ...
            {                
                ...
                localMarkers = new Dictionary<string, MapMarker>();
            ...
            public void AddMarkerL(string id, MapMarker marker)
            { 
                if (!localMarkers.ContainsKey(id))
                    localMarkers.Add(id, marker);

            }

            public void DelMarkerL(string id)
            {
                MapMarker m;
                if (localMarkers.TryGetValue(id, out m))
                    localMarkers.Remove(id);
            ...
            private void UpdateMarker()
            {
                ...
                marker = new MapMarker { name = RemoveSpecialCharacters(player.displayName), r = GetDirection(player?.eyes?.rotation.eulerAngles.y ?? 0), x = GetPositionx(transform.position.x), z = GetPositionz(transform.position.z) };
        ...
        class ActiveEntity : MonoBehaviour
        {
            ...
            void UpdatePosition()
            {
                ...
                marker.x = GetPositionx(entity.transform.position.x);
                marker.z = GetPositionz(entity.transform.position.z);
        ...
        {
            static public bool minimap, complexmap, monuments, names, compass, caves, plane, heli, supply, debris, player, allplayers, friends, vending, forcedzoom, cars, tanks, team;
        ...
        void CreateStaticMain()
        {
            ...
            LustyUI.MainMin = "0.2771875 0.115";
            LustyUI.MainMax = "0.7228125 0.885";
        ...
        void UpdateOverlay(BasePlayer player, string panel, string posMin, string posMax, float iconsize)
        {
            ...
            if (MapSettings.team) 
            { 
                List<string> teammates = getTeamMembers(player.UserIDString);
                
                foreach(var teammateid in teammates){
                    if (mapUsers.ContainsKey(teammateid)){
                        var teammate = mapUsers[teammateid];
                        if (teammate.InEvent() && configData.Map.HideEventPlayers) continue;
                        var marker = teammate.GetMarker();
                        if (marker == null) continue;
                        var image = GetImage($"friend{marker.r}");
                        if (string.IsNullOrEmpty(image)) continue;
                        AddIconToMap(ref mapContainer, panel, image, marker.name, iconsize * 1.25f, marker.x, marker.z);
            ...
            foreach(var mark in user.localMarkers.Values){
                AddIconToMap(ref mapContainer, panel, GetImage(mark.icon), "", iconsize * 1.25f, mark.x, mark.z);
                //Debug.Log( GetImage(mark.icon) +  mark.name +  (iconsize * 1.25f) + mark.x + mark.z);
            }
        ...
        private void LoadSettings()
        {
            ...
            MapSettings.team = configData.Markers.ShowTeam;
        ...
        private void FindStaticMarkers()
        {
            ...
            { 
                ...
                {                    
                    ...
                    {
                        x = GetPositionx(monument.transform.position.x),
                        z = GetPositionz(monument.transform.position.z)
                    };
        ...
        static float GetPosition(float pos) => (pos + mapSize / 1.95f) / mapSize;
        static float GetPositionx(float pos) => (pos + mapSize / 2f) / mapSize;
        static float GetPositionz(float pos) => (pos + mapSize / 2f) / mapSize;
        ...
        bool AddLocalMarker(BasePlayer player, float x, float z, string name, string icon = "special", float r = 0)
        {
            ...
            if (user.localMarkers.ContainsKey(name)) return false;
            MapMarker marker = new MapMarker
            {
                icon = icon,
                name = name,
            ...
            if (!string.IsNullOrEmpty(icon) && icon != "special")
            {
                ...
                if (!url.StartsWith("http") && !url.StartsWith("www") && !url.StartsWith("file://"))
                    url = $"{dataDirectory}custom{Path.DirectorySeparatorChar}{icon}.png";
                ImageLibrary.AddImage(url, icon, 0);
            }
            
            user.AddMarkerL(name, marker);
            
            return true;

        }
        bool RemoveLocalMarker(BasePlayer player, string name)
        {
            if (!activated) return false;
        ...
        class ConfigData
        {
            ...
            public class MapMarkers
            {
                ...
                [JsonProperty(PropertyName = "Show teammates")]
