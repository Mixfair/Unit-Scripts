...
namespace Oxide.Plugins 
{
    ...
    class NoEscape : RustPlugin
    {
        ...
        void OnServerInitialized ()
        {
            ...
            teamShare = GetConfig ("Raid", "BlockWho", "team", false);
        ...
        public class RaidZone
        {
            ...
            public Timer timer;
            public DateTime whenBlock;

            public RaidZone (string zoneid, Vector3 position, DateTime lastBlock)
            {
                this.zoneid = zoneid;
                this.position = position;
                this.whenBlock = lastBlock;
            }
        ...
        void StructureAttack (BaseEntity targetEntity, BaseEntity sourceEntity, string weapon, Vector3 hitPosition)
        {
            ...
            if (debug) Debug.Log(targetEntity.transform.position.ToString());
            ...
            if (targetEntity.OwnerID.IsSteamId () || (blockUnowned && !targetEntity.OwnerID.IsSteamId ())) {
                ...
                if (ownerBlock)
                    OwnerBlock (source, targetEntity, sourceMembers);

                if (raiderBlock)
                    RaiderBlock (source, targetEntity);
        ...
        void OwnerBlock (BasePlayer source, BaseEntity raidedEntity, List<string> sourceMembers = null)
        {
            //if (!ShouldBlockEscape (raidedEntity.OwnerID, source.userID))
             //   return;

            var targetMembers = new List<string> ();

            if (cupboardShare)
                targetMembers.AddRange(CupboardShare(raidedEntity));

            if (debug) Debug.Log("OwnerBlock called! With cupboard:");
            if (targetMembers != null) {
                foreach (var member in targetMembers) {
                      if (debug) Debug.Log(member);
                  }
            } 
           
            targetMembers = targetMembers.Distinct().ToList();

            if (targetMembers.Count > 0) {
                foreach (string targetid in targetMembers) {
                    BasePlayer plytarg = RelationshipManager.FindByID(Convert.ToUInt64(targetid));
                    if (plytarg == null || plytarg.IsNpc) continue;
                    StartRaidBlocking (plytarg, raidedEntity.transform.position);
                }
            }
        ...
        List<string> CupboardShare (BaseEntity raidedEntity)
        {
            
            List<string> sourceMembers = new List<string> ();
            var cup = raidedEntity.GetBuildingPrivilege();

            if (cup == null) 
                return sourceMembers;            

            List<string> cupboardMembers = new List<string> (); 
            List<string> ownerSupport = new List<string> (); 

            if (debug) Debug.Log(cup);
            foreach (var proto in cup.authorizedPlayers){
                if (debug) Debug.Log("Found cupboard user " + proto.userid.ToString());
                cupboardMembers.Add(proto.userid.ToString()); 
                if (clanShare || friendShare || teamShare){
                    ownerSupport.AddRange(getFriends(proto.userid.ToString()));
                }
            }

            sourceMembers.AddRange (cupboardMembers);
            sourceMembers.AddRange (ownerSupport);

            return sourceMembers;
        }

        void RaiderBlock (BasePlayer source, BaseEntity raidedEntity)
        {
            if (!ShouldBlockEscape (raidedEntity.OwnerID, source.userID))
                return;
            ...
            targetMembers.Add(source.userID.ToString()); 
            

            if (clanShare || friendShare || teamShare)
                targetMembers.AddRange(getFriends(source.UserIDString));
                    
            
            // if (debug) Debug.Log("RaiderBlock called! With friends/clan/team:");
            // if (targetMembers != null) {
            //     foreach (var member in targetMembers) {
            //           if (debug) Debug.Log(member);
            //       }
        ...
        { 
            ...
            LustyMap?.Call ("AddLocalMarker", player, player.transform.position.x, player.transform.position.z, "sdsds", LustyMapIcon);
            //CreateRaidZone (player.transform.position, player);
        ...
        void cmdBUnblocked (BasePlayer player, string command, string [] args)
        {
            ...
            LustyMap?.Call ("RemoveLocalMarker", player, "sdsds");
        ...
        // void StartRaidBlocking (BasePlayer player, Vector3 zone, bool createZone, string zoneId)
        // {
        //     StartRaidBlocking (player, zone, createZone, zoneId, player.transform.position)
        // }
        void StartRaidBlocking (BasePlayer target, Vector3 position, bool createZone = true, string zoneid = null)
        {
            ...
            if (zoneid!=null) {
                ...
                if (zones.TryGetValue (zoneid, out zone)) {
                    ...
                    raidBlocker.lastBlock = zone.whenBlock;
            ...
            if (debug) Debug.Log("Block Starting on " + target.ToString());
            
            //Debug.Log(raidBlockNotify); 

            if (raidBlockNotify)
                SendBlockMessage (target, raidBlocker, "Raid Block Notifier", "Raid Block Complete", position);

            if (useZoneManager && createZone && (zoneEnter || zoneLeave))
                CreateRaidZone (position, target);
        }
        ...
        void ResetZoneTimer (RaidZone zone)
        {
            zone.ResetTimer ().timer = timer.In (raidDuration, delegate () {
             ...
             Debug.Log("ZoneReseted");

        } 
  
        void CreateRaidZone (Vector3 position, BasePlayer raider)
        { 
            var zoneid = position.ToString ();
            Debug.Log("ZoneCreated");
            ...
            foreach (var nearbyZone in zones) {
                if (nearbyZone.Value.Distance (position) < (raidDistance / 2)) {
                    var zid = nearbyZone.Value.zoneid;
                    nearbyZone.Value.ResetTimer();
            ...
            zones.Add (zoneid, zone = new RaidZone (zoneid, position, raider.gameObject.GetComponent<RaidBlock>().lastBlock));
        ...
        {   
            ...
            if (debug) Debug.Log("Entered in zone " + zoneId + " id: " +player.ToString());
            Vector3 zonePos = ZoneManager.Call<Vector3>("GetZoneLocation", zoneId);
            Debug.Log(zonePos);
            StartRaidBlocking (player, zonePos, false, zoneId);
        }
        ...
        public List<string> getFriends (string player)
        {
            ...
            if (teamShare) {
                var members = getTeamMembers (player);
                if (members != null)
                    players.AddRange (members);
            }
        ...
        public List<string> getTeamMembers(string player){

            if (debug) Debug.Log("Trying find teammates.." + player);

            var players = new List<string> ();

            RelationshipManager.PlayerTeam playersTeam = RelationshipManager.Instance.FindPlayersTeam(Convert.ToUInt64(player));
            
            
            if (playersTeam != null)
                foreach (ulong member in playersTeam.members)
                {
                    if (Convert.ToString(member) != player){
                        if (debug) Debug.Log("Found teammate :" + Convert.ToString(member));
                        players.Add(Convert.ToString(member));
                    }
        ...
        void SendBlockMessage (BasePlayer target, BlockBehavior blocker, string langMessage, string completeMessage, Vector3 raidedpos = new Vector3())
        {
            var send = false;
            ...
            if (send) {
                ...
                if (sendChatNotification || sendGUIAnnouncementsNotification)
                    message = GetPrefix (target.UserIDString) + GetMsg (langMessage, target.UserIDString).Replace ("{time}", GetCooldownTime(blocker.Duration - ts.TotalSeconds, target.UserIDString));
                ...
                if (sendLustyMapNotification && LustyMapDuration > 0) {
                    //RaidZone rz = GetZoneByID(zoneId); 
                    //Vector3 raidedpos = rz.position;
                    var id = Guid.NewGuid ().ToString ("N"); 
                    var obj = LustyMap?.Call ("AddLocalMarker", target, raidedpos.x, raidedpos.z, id, LustyMapIcon);
                    if (obj is bool && (bool)obj == true) {
                        timer.In (LustyMapDuration, delegate () {
                            LustyMap?.Call ("RemoveLocalMarker", target, id);
                        });
                    }
                }
            ...
            blocker.Notify (delegate () {
                ...
                if (target?.IsConnected == true) {
                    ...
                    //if (sendLustyMapNotification && LustyMapDuration > 0)
                       // LustyMap?.Call ("RemoveTemporaryMarker", blocker.iconUID);
                }
            });
        }

        string GetCooldownTime (double f, string userID)
        {
            Debug.Log(f);
            if (f > 60)
                return Math.Round (f / 60, 1) + " " + GetMsg ("Unit Minutes", userID);

            return Math.Round(f) + " " + GetMsg ("Unit Seconds", userID);
        }
